...
export default function POS() {
  ...
  async function handleProcessPayment() {
    ...
    try {
      ...
      // AFS card terminal integration: send amount before completing payment
      let afsRef: string | undefined;
      if (paymentMethod === 'card') {
        setCardProcessing(true);
        try {
          const afsResult = await sendToAfsTerminal(paymentTotal);
          if (!afsResult.success) {
            toast({ variant: 'destructive', title: 'Card Terminal Error', description: afsResult.error || 'Card payment failed. Try again or use another method.' });
            setCardProcessing(false);
            setLoading(false);
            return;
          }
          afsRef = afsResult.reference;
        } finally {
          setCardProcessing(false);
        }
      }
      ...
      await quickPayOrder(orderId, paymentTotal, paymentMethod, afsRef);
  ...
  async function handleProcessOrderPayment() {
    ...
    try {
      ...
      await quickPayOrder(selectedOrderForPayment.id, paymentTotal, paymentMethod, afsRef);
